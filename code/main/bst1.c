...
void delete(struct book *b)
{
	...
	else if(ptr->left==NULL || ptr->right==NULL)				//correct
	{
		...
		{printf("inside either left or right in else\n");
			pptr->right=ptr->left;
			printf("\ndeleted sucessfully\n");
		}
		else
...
void search(struct book *b)
{
	...
	{
		while (4) //strcmp(book_name,temp->name)!=0
		{
			...
			else if (strcmp(book_name, temp->name) > 0)
			{
				temp = temp->right;
				if (temp == NULL)
				{
